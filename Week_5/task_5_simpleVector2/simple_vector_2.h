...
public:
  ...
  SimpleVector(SimpleVector &&other) noexcept : start_(new T[other.capacity_]), size_(other.size_), capacity_(other.capacity_)
  {
  ...
  {
    if (size_ >= capacity_) {
      const size_t new_cap = capacity_ == 0 ? 1 : 2 * capacity_;
