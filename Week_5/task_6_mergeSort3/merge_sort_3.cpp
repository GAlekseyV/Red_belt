...
void MergeSort(RandomIt range_begin, RandomIt range_end)
{
  ...
  RandomIt endPart1_it = vec.begin() + vec.size() / 3;
  RandomIt endPart2_it = endPart1_it + vec.size() / 3;
  // Вызвать функцию MergeSort от каждой части вектора.
  MergeSort(vec.begin(), endPart1_it);
  MergeSort(endPart1_it, endPart2_it);
  MergeSort(endPart2_it, vec.end());
  ...
  merge(make_move_iterator(begin(vec)), make_move_iterator(endPart1_it), make_move_iterator(endPart1_it), make_move_iterator(endPart2_it), back_inserter(tmp_v));
  ...
  merge(make_move_iterator(begin(tmp_v)), make_move_iterator(end(tmp_v)), make_move_iterator(endPart2_it), make_move_iterator(end(vec)), range_begin);
}
...
{
  TestRunner trunner;
  RUN_TEST(trunner, TestIntVector);
  return 0;
}
